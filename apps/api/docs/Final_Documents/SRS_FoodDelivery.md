...
| _(4)_ | _BR-1.2_ | **Authenticate Rules:**<br>❖ `email` must be a valid email format. `password` must be non-empty.<br>❖ System queries the user repository by email; if not found, returns HTTP 401 referencing `MSG-AUTH-01`.<br>❖ System compares submitted password hash against stored hash; mismatch returns HTTP 401 referencing `MSG-AUTH-01`.<br>❖ No distinction is made between missing-email and wrong-password failures to prevent user enumeration.<br>❖ If `AuthService.signInWithEmail(email, password)` and `UserRepository.findUserByEmail(email)` returns false, system returns an error response referencing `MSG-AUTH-01`. |
...
| _(5)_ | _BR-1.4_ | **Session Creation Rules:**<br>❖ Upon successful authentication, system creates a session record with `expiresAt = now + sessionTtl` (default 7 days).<br>❖ `ipAddress` and `userAgent` are captured for audit purposes.<br>❖ Session token is returned to client as Bearer token for all subsequent authenticated requests.<br>❖ After `AuthService.createSession(userId, ipAddress, userAgent)` completes, session token returned to client. |
| _(8)_ | _BR-1.5_ | **Registration Form Rules:**<br>❖ System collects `name` (full name), `email`, and `password` from the registration form. |
| _(9)_ | _BR-1.6_ | **Validate Rules (Sign Up):**<br>❖ `name` must be a non-empty string.<br>❖ `email` must be a valid email format.<br>❖ `password` must be at least 8 characters and contain at least one letter and one digit.<br>❖ Invalid input returns HTTP 400 referencing `MSG-AUTH-03`.<br>❖ If `AuthService.signUpWithEmail(email, password, name)` returns false, system returns an error response referencing `MSG-AUTH-03`. |
| _(12)_ | _BR-1.7_ | **Duplicate Email Rules:**<br>❖ System checks if `email` is already registered in the user repository.<br>❖ If duplicate found, system returns HTTP 409 referencing `MSG-AUTH-02`.<br>❖ If `UserRepository.findUserByEmail(email)` returns null or empty, system returns an error response referencing `MSG-AUTH-04` (email already registered). |
...
| _(14)_ | _BR-1.9_ | **Reset Code Dispatch Rules:**<br>❖ System generates a single-use OTP valid for 60 minutes and stores it in the verification repository.<br>❖ OTP is transmitted via the configured channel (email or SMS) to the registered email address.<br>❖ System responds HTTP 200 referencing `MSG-AUTH-04` regardless of whether the email exists in the repository (anti-enumeration).<br>❖ After `AuthService.requestPasswordReset(email)` completes, reset code dispatched to recipient mailbox. |
...
| _(17)_ | _BR-1.11_ | **Logout Rules:**<br>❖ System immediately deletes the session record from the repository.<br>❖ Subsequent requests bearing the invalidated token receive HTTP 401 referencing `MSG-AUTH-05`.<br>❖ After `AuthService.signOut(sessionId)` completes, session revoked. |
| _(2)_ | _BR-1.12_ | **Session Validation Rules:**<br>❖ Every protected endpoint requires a valid non-expired Bearer token in the `Authorization` header.<br>❖ Missing, malformed, or expired token returns HTTP 401 referencing `MSG-AUTH-05`.<br>❖ Valid token attaches the session's user to the request context for authorization checks.<br>❖ If `AuthService.validateSession(token)` fails the authorization check, system returns HTTP 403 referencing `MSG-AUTH-08`. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(3)_ | _BR-2.1_ | **Validate Rules:**<br>❖ If `lat` or `lon` is supplied, both must be present together. Missing one while providing the other returns HTTP 400 referencing `MSG-DISC-01`.<br>❖ `radiusKm` (if supplied) must be a positive number.<br>❖ If `SearchService.validateSearchQuery(q, lat, lon, radiusKm)` returns false, system returns an error response referencing `MSG-DISC-01`. |
...
| _(4)_ | _BR-2.3_ | **Pagination Rules:**<br>❖ `limit` is clamped to [1, 100]. `offset` is clamped to ≥ 0.<br>❖ Requests outside these bounds are silently corrected and processed with clamped values (no error returned).<br>❖ If `SearchService.clampPagination(limit, offset)` returns false, system returns an error response referencing `MSG-DISC-02` (pagination params normalized). |
| _(4)_ | _BR-2.4_ | **Restaurant Query Rules:**<br>❖ Only restaurants with `isApproved = true` AND `isOpen = true` are included in results.<br>❖ If `lat`, `lon`, and `radiusKm` are provided, filtering applies Haversine distance and returns only restaurants within the specified radius.<br>❖ When `RestaurantRepository.findAll(offset, limit)` and `SearchService.searchRestaurants(q, lat, lon, radiusKm)` executes, matching restaurants returned. |
| _(4)_ | _BR-2.5_ | **Item Query Rules:**<br>❖ Only items with `status = 'available'` are included in search results.<br>❖ Items are populated only when at least one search criterion (`q`, `category`, or `tag`) is provided; otherwise empty array is returned.<br>❖ When `SearchService.searchItems(q, category, tag)` executes, matching items returned. |
| _(4)_ | _BR-2.6_ | **Relevance Scoring Rules:**<br>❖ Restaurants: exact name match +12 points, partial name match +9, cuisine match +6, description partial match +2.<br>❖ Items: exact name match +12 points, partial name match +8, tag match +5, category match +3.<br>❖ Results are sorted by score descending; ties broken by stable UUID ordering.<br>❖ After `SearchService.scoreAndRank(results, query)` completes, ranked result set returned to caller. |
...
| _(3)_ | _BR-4.2_ | **Item Availability Rules:**<br>❖ System fetches the Ordering ACL snapshot for the restaurant. If no snapshot is found, returns HTTP 400 referencing `MSG-CART-01`.<br>❖ Item `status` must be `'available'` within the snapshot. If not, returns HTTP 409 referencing `MSG-CART-02`.<br>❖ If `OrderingAcl.loadMenuItemSnapshot(menuItemId)` returns null or empty, system returns an error response referencing `MSG-CART-02` (item unavailable). |
...
| _(3)_ | _BR-4.4_ | **Modifier Validation Rules:**<br>❖ Each `(groupId, optionId)` pair must exist and be available on the snapshot.<br>❖ Per-group selection count must satisfy `minSelections ≤ count ≤ maxSelections`.<br>● Modifier group not found → HTTP 400 + `MSG-CART-03`.<br>● Modifier option not found → HTTP 400 + `MSG-CART-04`.<br>● Option unavailable → HTTP 400 + `MSG-CART-05`.<br>● Below minimum selections → HTTP 400 + `MSG-CART-06`.<br>● Exceeds maximum selections → HTTP 400 + `MSG-CART-07`.<br>❖ If `OrderingAcl.validateModifiers(menuItemId, selections)` returns false, system returns an error response referencing `MSG-CART-03`. |
...
| _(5)_ | _BR-4.8_ | **Line Item Merge Rules:**<br>❖ Line item identity is defined by `(menuItemId, modifierFingerprint)`. Adding the same item with identical modifiers increments the existing line's quantity.<br>❖ Per-line ceiling is 99 units. Adding would exceed this → HTTP 400 referencing `MSG-CART-10`.<br>❖ After `CartService.mergeLineItem(cart, item)` completes, line merged or appended in cart. |
...
| _(5)_ | _BR-5.15_ | **TTL Reset Rules (Update Qty):**<br>❖ Successful quantity update resets Redis TTL to 7 days. Read-only GET does not reset TTL.<br>❖ After `CartService.refreshCartTtl(customerId)` completes, cart TTL refreshed. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(2)_ | _BR-6.1_ | **Validate Address Structure:**<br>❖ `street`, `district`, and `city` are required non-empty strings. Invalid returns HTTP 400 referencing `MSG-ADDR-01`.<br>❖ If `AddressService.validateAddressDto(address)` returns false, system returns an error response referencing `MSG-ADDR-01`. |
| _(3)_ | _BR-6.2_ | **Validate Coordinate Pairing:**<br>❖ `latitude` and `longitude` are optional, but if either is provided both must be present. Invalid pairing returns HTTP 400 referencing `MSG-ADDR-02`.<br>❖ If `AddressService.validateCoordinatePair(lat, lon)` returns false, system returns an error response referencing `MSG-ADDR-02`. |
...
| _(6)_ | _BR-6.8_ | **Address Immutability Rules:**<br>❖ Upon order placement, the address is stored in `orders.delivery_address` JSONB column.<br>❖ Once stored, address cannot be changed. Correction requires order cancellation and re-placement.<br>❖ After `AddressService.snapshotAddressForOrder(orderId, address)` completes, address snapshot frozen on order. |
| _(1)_ | _BR-6.9_ | **Address Book Lifecycle Rules:**<br>❖ Customer addresses are managed (created, updated, deleted, listed) through the address capability.<br>❖ At checkout, the selected or entered address is bound to the order snapshot for immutable fulfillment.<br>❖ After `AddressService.createAddress(customerId, label, line, lat, lon)` and `AddressService.listAddresses(customerId)` completes, address record persisted in address book. |
...
| _(6)_ | _BR-7.7_ | **ACL Synchronization (Create / Update):**<br>❖ Every successful Create or Update publishes a `DeliveryZoneSnapshotUpdatedEvent`.<br>❖ Ordering ACL projector upserts the corresponding snapshot row. UC-8 reads zone data exclusively from this ACL projection.<br>❖ After `RestaurantService.create(restaurantId, requesterId, isAdmin, dto)` and `RestaurantService.update(id, restaurantId, requesterId, isAdmin, dto)` completes, `EventPublisher.publishRestaurantUpdatedEvent(restaurantId)` publishes the corresponding domain event (ACL snapshot synchronised in ordering BC). |
| _(9)_ | _BR-7.8_ | **ACL Synchronization (Delete):**<br>❖ Every successful Delete publishes a `DeliveryZoneSnapshotUpdatedEvent`.<br>❖ Ordering ACL projector removes the snapshot row.<br>❖ After `ZonesService.remove(id, restaurantId, requesterId, isAdmin)` completes, `EventPublisher.publishRestaurantUpdatedEvent(restaurantId)` publishes the corresponding domain event (ACL snapshot invalidated in ordering BC). |
| _(13)_ | _BR-7.9_ | **Estimate Precondition Validation:**<br>❖ System verifies restaurant has configured `latitude` / `longitude` and at least one active delivery zone.<br>❖ Missing configuration returns HTTP 422 referencing `MSG-ZONE-04`.<br>❖ If `ZonesService.estimateDelivery(restaurantId, query)` returns false, system returns an error response referencing `MSG-ZONE-03`. |
...
| _(15)_ | _BR-7.13_ | **Fee and ETA Calculation Rules:**<br>❖ `shippingFee = round((baseFee + distanceKm × perKmRate) / 1000) × 1000` (nearest 1,000 VND).<br>❖ `estimatedDeliveryMinutes = ceil(prepTimeMinutes + (distanceKm / avgSpeedKmh) × 60 + bufferMinutes)`.<br>❖ After `ZonesService.calculateFeeAndEta(zone, distance)` completes, fee and ETA returned in response payload. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(2)_ | _BR-8.1_ | **Validate Rules:**<br>❖ `paymentMethod` must be one of `{'cod', 'vnpay'}`.<br>❖ `deliveryAddress` must satisfy address validation rules (BR-6.1).<br>❖ `note` must be ≤ 500 characters.<br>❖ `X-Idempotency-Key` (if present) must be a UUID string of 8–64 hexadecimal characters with optional hyphens.<br>❖ Invalid input returns HTTP 400 referencing `MSG-ORD-01` (general validation) or `MSG-ORD-02` (idempotency key format).<br>❖ If `CheckoutValidator.validateCheckoutDto(body)` returns false, system returns an error response referencing `MSG-ORD-01`. |
...
| _(3)_ | _BR-8.3_ | **Idempotency Rules:**<br>❖ If an `X-Idempotency-Key` is present and a cached `orderId` already exists for that key, system returns the existing order response without re-processing.<br>❖ The idempotency record is written after successful order persistence so that partial failures do not cache a stale state.<br>❖ If `IdempotencyService.checkIdempotency(idempotencyKey, customerId)` returns null or empty, system returns an error response referencing `MSG-ORD-02` (replay returns previously stored response). |
...
| _(7)_ | _BR-8.13_ | **Promotion Reservation Rules:**<br>❖ Promotion reservation is non-blocking; if reservation fails or returns `discountAmount = 0`, checkout continues without a discount.<br>❖ Promotion usage is confirmed after successful order persistence; failures in confirmation are reconciled by the promotion reconciliation task.<br>❖ After `PromotionService.computeAndReserveDiscount(orderId, customerId, items)` executes, reservation succeeds and discount applied. |
| _(8)_ | _BR-8.14_ | **Server-Authoritative Pricing Rules:**<br>❖ Order line `unitPrice` and modifier prices are taken from ACL snapshots at checkout time, not from values stored at add-to-cart time.<br>❖ `itemsTotal` must be > 0; if ≤ 0, return HTTP 422 referencing `MSG-ORD-12`.<br>❖ `totalAmount = max(0, itemsTotal + shippingFee − discountAmount)`.<br>❖ After `PricingService.computeOrderTotals(items, fees, discount)` completes, server-side totals override any client-supplied values. |
...
| _(9)_ | _BR-8.19_ | **Payment Transaction Recording Rules:**<br>❖ For `paymentMethod = 'vnpay'`: a `payment_transactions` row is created with `status = 'pending'`.<br>❖ For `paymentMethod = 'cod'`: no payment transaction is created at checkout; payment is collected at delivery.<br>❖ VNPay URL generation failure is logged but non-blocking; payment timeout reconciliation is handled by UC-9 (BR-9.11).<br>❖ After `PaymentService.recordPaymentTransaction(orderId, amount, method)` completes, `pending` payment transaction persisted alongside order. |
| _(10)_ | _BR-8.20_ | **VNPay URL Generation Rules:**<br>❖ For `paymentMethod = 'vnpay'` at activity (9), a VNPay redirect URL is generated and included in the response payload.<br>❖ For `paymentMethod = 'cod'`, no URL is generated; checkout proceeds with next activity.<br>❖ If `VNPayService.buildVNPayPaymentUrl(orderId, amount, ipAddr)` returns success, VNPay redirect URL returned to client; otherwise the system records the failure and surfaces it to the caller. |
| _(11)_ | _BR-8.21_ | **Post-Persistence Rules:**<br>❖ `OrderPlacedEvent` is published exactly once after successful persistence.<br>❖ Cart deletion (`cart:<customerId>`) is best-effort; failure does not invalidate the order.<br>❖ Successful response: HTTP 201 referencing `MSG-ORD-15` with payload `{ orderId, status: 'pending', totalAmount, shippingFee, discountAmount, paymentUrl?, estimatedDeliveryMinutes? }`.<br>❖ `EventPublisher.publishOrderPlacedEvent(orderId, customerId, totalAmount)` publishes the corresponding domain event. |
| _(12)_ | _BR-8.22_ | **Order Confirmation Response Rules:**<br>❖ After successful order persistence at activity (11), system returns HTTP 201 with order confirmation payload and `MSG-ORD-15`.<br>❖ After `CheckoutController.checkout(session, headers, body)` completes, confirmation payload with `orderId` and `paymentUrl` returned. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-9.1_ | **Signature Verification Rules:**<br>❖ The IPN signature is verified using HMAC-SHA512 over the sorted VNPay query parameters with the merchant secret key, using constant-time comparison to prevent timing attacks.<br>❖ Signature verification is the mandatory first step before any database access.<br>❖ Invalid signature returns `MSG-PAY-01` (RspCode 97).<br>❖ If `VNPayService.verifyVNPaySignature(payload, secureHash)` returns false, system returns an error response referencing `MSG-PAY-01` (signature invalid). |
...
| _(8)_ | _BR-9.7_ | **Amount Integrity Rules:**<br>❖ `vnp_Amount` divided by 100 must exactly match `payment_transactions.amount`.<br>❖ Any mismatch marks the transaction `'failed'` and returns `MSG-PAY-03` (RspCode 04).<br>❖ If `PaymentService.assertAmountMatches(orderAmount, ipnAmount)` returns false, system returns an error response referencing `MSG-PAY-04` (amount mismatch). |
...
| _(13)_ | _BR-11.8_ | **Persistence Rules (Profile Update):**<br>❖ The profile update is applied to the restaurant row; all modified fields are persisted atomically.<br>❖ After `RestaurantService.update(id, session, dto)` completes, restaurant profile updated. |
| _(14)_ | _BR-11.9_ | **Event Synchronisation Rules (Profile Update):**<br>❖ Every successful profile update publishes `RestaurantUpdatedEvent` containing the latest persisted state.<br>❖ The event is consumed by the Ordering ACL projector to refresh the restaurant snapshot used by UC-8 and ownership checks in UC-14 / UC-15.<br>❖ `EventPublisher.publishRestaurantUpdatedEvent(restaurantId)` publishes the corresponding domain event. |
...
| _(8)_ | _BR-11.17_ | **Restaurant Record Creation Rules:**<br>❖ A new `restaurants` row is created with `ownerId = session.user.id`, `isApproved = false`, `isOpen = false`.<br>❖ After `RestaurantService.create(session, dto)` completes, new restaurant record created with `isApproved=false`. |
...
| _(20)_ | _BR-11.20_ | **Approval Workflow Rules:**<br>❖ Administrator reviews application documents and chooses to approve or unapprove the restaurant.<br>❖ After `RestaurantService.approve(id)` and `RestaurantService.unapprove(id)` succeeds, approval state toggled by admin decision. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-12.1_ | **Validate Rules (Menu Item):**<br>❖ `name` is required and non-empty.<br>❖ `price` is required and must be a non-negative integer (VND, no fractional units).<br>❖ `restaurantId` is required and must reference an existing restaurant.<br>❖ `categoryId` (if supplied) must reference a category that belongs to the same `restaurantId`.<br>❖ `tags` (if supplied) must be an array of non-empty strings.<br>❖ Invalid input returns HTTP 400 referencing `MSG-MENU-01`.<br>❖ If `MenuService.validateMenuItemDto(dto)` returns false, system returns an error response referencing `MSG-MENU-01`. |
| _(5)_ | _BR-12.2_ | **Authorization Rules (Menu Mutation):**<br>❖ Catalog mutations (`POST`, `PATCH`, `DELETE`) require role `restaurant` or `admin`.<br>❖ For role `restaurant`, mutations are allowed only when the owning restaurant's `ownerId = session.user.id`; otherwise HTTP 403 referencing `MSG-MENU-05`.<br>❖ If `MenuService.assertMenuOwnership(restaurantId, requesterId, isAdmin)` fails the authorization check, system returns HTTP 403 referencing `MSG-MENU-02`. |
| _(6)_ | _BR-12.3_ | **Validate Rules (Menu Category):**<br>❖ `name` is required and non-empty.<br>❖ `displayOrder` (if supplied) must be a non-negative integer.<br>❖ The pair `(restaurantId, name)` must be unique; duplicate returns HTTP 409 referencing `MSG-MENU-03`.<br>❖ Invalid input returns HTTP 400 referencing `MSG-MENU-01`.<br>❖ If `MenuService.validateCategoryDto(dto)` returns false, system returns an error response referencing `MSG-MENU-03`. |
...
| _(10)_ | _BR-12.7_ | **Catalog Persistence Rules:**<br>❖ The catalog row is created, updated, or removed in the corresponding table (`menu_categories`, `menu_items`, `modifier_groups`, `modifier_options`).<br>❖ After `RestaurantService.create(requesterId, isAdmin, dto)` and `RestaurantService.update(id, requesterId, isAdmin, dto)` completes, menu item or category persisted. |
...
| _(4)_ | _BR-12.15_ | **Validate Rules (Modifier Group & Option):**<br>❖ Modifier group: `minSelections` ≥ 0 and `maxSelections` ≥ `minSelections`; otherwise HTTP 400 referencing `MSG-MENU-06`.<br>❖ Modifier option: `price` is a non-negative integer (VND); `0` denotes a free option.<br>❖ Option must belong to a modifier group that belongs to the same menu item in the URL; otherwise HTTP 404 referencing `MSG-MENU-07`.<br>❖ If `ModifiersService.createGroup(menuItemId, session, dto)` and `ModifiersService.updateGroup(menuItemId, id, session, dto)` returns false, system returns an error response referencing `MSG-MENU-04`. |
...
| _(14)_ | _BR-13.8_ | **Restaurant Open/Close Persistence Rules:**<br>❖ `isOpen` is set atomically; the new value persists and controls "currently accepting orders" for the restaurant.<br>❖ After `RestaurantService.update(id, session, dto)` completes, restaurant `isOpen` flag persisted. |
...
| _(16)_ | _BR-13.10_ | **Propagation Rules (Restaurant Status):**<br>❖ Every successful open/close operation publishes `RestaurantUpdatedEvent` with the new `isOpen` flag.<br>❖ The Ordering ACL projector refreshes its snapshot.<br>❖ `EventPublisher.publishRestaurantUpdatedEvent(restaurantId)` publishes the corresponding domain event. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-14.1_ | **Session & Role Resolution Rules:**<br>❖ Session is verified and actor role (`restaurant`, `admin`, or `customer`) is resolved from authentication context.<br>❖ If `OrderLifecycleService.assertRestaurantRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-01`. |
| _(6)_ | _BR-14.2_ | **Order Loading Rules:**<br>❖ The order is loaded by id; a non-existent id returns HTTP 404 referencing `MSG-HIST-01`.<br>❖ If the order is already in the requested target status, system returns the order unchanged (no new audit row, no event).<br>❖ If `OrderRepository.findOrderById(orderId)` returns null or empty, system returns an error response referencing `MSG-LCYC-02` (order not found). |
...
| _(8)_ | _BR-14.4_ | **Transition Validity Rules:**<br>❖ Allowed transitions for this UC: T-01 (`pending → confirmed`), T-03 (`pending → cancelled`), T-04 (`paid → confirmed`), T-05 (`paid → cancelled`), T-07 (`confirmed → cancelled`).<br>❖ T-03 and T-05 additionally allow roles `customer` and `system` per the TRANSITIONS map.<br>❖ Any other `(fromStatus, toStatus)` pair returns HTTP 422 referencing `MSG-LCYC-01`.<br>❖ Each transition has an `allowedRoles` set; a role outside the set returns HTTP 403 referencing `MSG-LCYC-02`.<br>❖ If `OrderLifecycleService.assertTransition(fromStatus, toStatus)` returns false, system returns an error response referencing `MSG-LCYC-01` (transition invalid). |
| _(10)_ | _BR-14.5_ | **Restaurant Ownership Rules:**<br>❖ For role `restaurant`, the target order's `restaurantId` must belong to a restaurant whose `ownerId = session.user.id` (resolved via Ordering ACL snapshot); otherwise HTTP 403 referencing `MSG-LCYC-03`.<br>❖ For role `admin`, the ownership check is bypassed.<br>❖ If `OrderLifecycleService.assertOrderOwnedByRestaurant(orderId, restaurantId)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-02`. |
...
| _(18)_ | _BR-14.10_ | **Atomicity & Concurrency Rules:**<br>❖ Status update and audit log insert occur in a single database transaction.<br>❖ Status update uses optimistic locking (`WHERE id = :id AND version = :loaded_version`); a zero-row result returns HTTP 409 referencing `MSG-LCYC-06`.<br>❖ `order_status_logs` row records `triggeredBy`, `triggeredByRole`, and `note`.<br>❖ After `OrderLifecycleService.confirm(id, session)` and `OrderLifecycleService.cancelOrder(id, session, dto)` succeeds, order status, audit row and event publication commit atomically. |
| _(19)_ | _BR-14.11_ | **Audit Log Creation Rules:**<br>❖ A new `order_status_logs` row is inserted atomically with the status update, recording `fromStatus`, `toStatus`, `triggeredBy`, `triggeredByRole`, and `note` (if any).<br>❖ After `OrderRepository.appendOrderHistory(orderId, status, actorId)` completes, audit row persisted with actor identity and timestamp. |
...
| _(24)_ | _BR-14.15_ | **Refund Initiation Rules:**<br>❖ If both conditions are met at activity (23), `OrderCancelledAfterPaymentEvent` is published to trigger the refund pipeline (UC-25).<br>❖ If `PaymentService.refund(id, session, dto)` returns success, refund flow initiated for VNPay-paid order; otherwise the system records the failure and surfaces it to the caller. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-15.1_ | **Authorization Rules:**<br>❖ T-06 requires role `restaurant` or `admin`; other roles return HTTP 403 referencing `MSG-LCYC-02`.<br>❖ If `OrderLifecycleService.assertRestaurantRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-01`. |
...
| _(13)_ | _BR-15.8_ | **State Transition Rules (T-08):**<br>❖ T-08 requires `order.status = 'preparing'`; any other source state returns HTTP 422 referencing `MSG-LCYC-01`.<br>❖ Idempotent re-issue: if already `ready_for_pickup`, returns unchanged without duplicate audit row or event.<br>❖ After `OrderLifecycleService.markReady(id, session)` succeeds, order transitions from `preparing` to `ready_for_pickup`. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(3)_ | _BR-16.1_ | **Input Validation Rules:**<br>❖ Full name, national ID, vehicle type, driving-licence image and (for non-bicycle) licence-plate number are required.<br>❖ Licence-plate format must match Vietnamese standards for the chosen vehicle type.<br>❖ Invalid input returns HTTP 400 referencing `MSG-SHIP-01`.<br>❖ If `ShipperOnboardingService.validateShipperRegistrationDto(dto)` returns false, system returns an error response referencing `MSG-SHIP-REG-01`. |
| _(4)_ | _BR-16.2_ | **Image Reference Validation Rules:**<br>❖ Uploaded ID and licence images must be existing image records owned by the applicant.<br>❖ Missing or unowned image references return HTTP 400 referencing `MSG-SHIP-01`.<br>❖ After `ShipperOnboardingService.uploadShipperDocument(shipperId, documentType, imageUrl)` completes, image reference stored on shipper application. |
| _(5)_ | _BR-16.3_ | **Duplicate Application Prevention:**<br>❖ An applicant with an existing `pending_approval` or `approved` application is rejected with HTTP 409 referencing `MSG-SHIP-02`.<br>❖ An applicant whose previous application was `rejected` may re-apply; the new row supersedes the old for queue purposes.<br>❖ If `ShipperOnboardingService.findPendingShipperApplicationByEmail(email)` returns null or empty, system returns an error response referencing `MSG-SHIP-REG-02` (duplicate pending application detected). |
...
| _(8)_ | _BR-16.6_ | **Admin Queue Access Rules:**<br>❖ Approve and reject endpoints are accessible only to users with role `admin`; otherwise HTTP 403 referencing `MSG-AUTH-05`.<br>❖ When `ShipperOnboardingService.listPendingShipperApplications(filters)` executes, pending applications returned for admin queue. |
...
| _(10)_ | _BR-16.8_ | **Approval Decision Gate:**<br>❖ Admin reviews application documents and makes an explicit Approve or Reject decision.<br>❖ The decision is recorded atomically with the status update in a single transaction.<br>❖ After `ShipperOnboardingService.approveShipper(shipperId, adminId, decision)` succeeds, admin decision recorded on application. |
| _(11)_ | _BR-16.9_ | **Role Elevation on Approval:**<br>❖ Upon approval, the applicant's account gains role `shipper` atomically with the application status update inside one DB transaction.<br>❖ A `ShipperApprovedEvent` is published after commit carrying `shipperId` and approved vehicle/plate information.<br>❖ After `RolePermissionService.assignRoleToUser(userId, role, adminId)` completes, shipper role granted to user account. |
| _(12)_ | _BR-16.10_ | **Shipper Activation Notification:**<br>❖ The published `ShipperApprovedEvent` notifies downstream contexts (notification, delivery dispatch) that the shipper is now eligible for UC-17 (availability) and UC-18 (pickup assignments).<br>❖ `EventPublisher.publishShipperApprovedEvent(shipperId)` publishes the corresponding domain event. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-17.1_ | **Session Verification Rules:**<br>❖ Endpoint requires an authenticated session with the caller's user id linked to an account holding role `shipper`.<br>❖ Non-authenticated requests return HTTP 401; non-shipper roles return HTTP 403 referencing `MSG-SHIP-04`.<br>❖ If `AuthService.validateSession(token)` fails the authorization check, system returns HTTP 403 referencing `MSG-AUTH-08`. |
| _(5)_ | _BR-17.2_ | **Shipper Account Approval Gate:**<br>❖ The shipper's underlying account must be in `approved` state (elevated during UC-16 admin approval).<br>❖ Pending or rejected applicants cannot change availability; return HTTP 403 referencing `MSG-SHIP-04`.<br>❖ If `ShipperService.assertShipperApproved(shipperId)` fails the authorization check, system returns HTTP 403 referencing `MSG-SHIP-AVAIL-01` (shipper not yet approved). |
...
| _(7)_ | _BR-17.4_ | **In-Flight Delivery Query:**<br>❖ System queries for any active order where `shipperId = actor.id` and `status ∈ {picked_up, delivering}`.<br>❖ If any row found, the offline transition is blocked.<br>❖ If `ShipperService.getShipperActiveOrder(shipperId)` returns null or empty, system returns an error response referencing `MSG-SHIP-AVAIL-02` (shipper has an active delivery). |
| _(8)_ | _BR-17.5_ | **Offline Transition Execution:**<br>❖ When no in-flight deliveries exist, the shipper's availability is set to `offline` atomically.<br>❖ The update completes in a single transaction with audit logging.<br>❖ After `ShipperService.setShipperAvailability(shipperId, isAvailable)` succeeds, shipper availability flag toggled. |
...
| _(11)_ | _BR-17.8_ | **Availability Change Event:**<br>❖ A successful change publishes `ShipperAvailabilityChangedEvent` carrying `shipperId`, new availability status, and the timestamp.<br>❖ `EventPublisher.publishShipperAvailabilityChangedEvent(shipperId, isAvailable)` publishes the corresponding domain event. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-18.1_ | **Shipper Role Gate:**<br>❖ Shipper role is required; other authenticated roles (`user`, `restaurant`, `admin` override) have different assignment flows.<br>❖ Non-shipper attempt returns HTTP 403 referencing `MSG-LCYC-02`.<br>❖ If `OrderLifecycleService.assertShipperRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-03`. |
| _(5)_ | _BR-18.2_ | **Order Existence Check:**<br>❖ Load order by id; non-existent orders return HTTP 404 referencing `MSG-HIST-01`.<br>❖ If `OrderRepository.findOrderById(orderId)` returns null or empty, system returns an error response referencing `MSG-LCYC-02` (order not found). |
| _(6)_ | _BR-18.3_ | **Ready-for-Pickup State Gate:**<br>❖ T-09 (`ready_for_pickup → picked_up`) requires the source state to be `ready_for_pickup`.<br>❖ Any other state returns HTTP 422 referencing `MSG-LCYC-01`.<br>❖ If `OrderLifecycleService.assertOrderStatus(orderId, expectedStatus)` returns false, system returns an error response referencing `MSG-LCYC-01` (order not in `ready_for_pickup` state). |
...
| _(10)_ | _BR-18.7_ | **Concurrency Control Initialization:**<br>❖ Load the order's current `version` for optimistic locking.<br>❖ Prepare the UPDATE statement with `version = :loaded_version` guard.<br>❖ If `LockService.acquireOrderLock(orderId)` returns false, system returns HTTP 409 referencing `MSG-LCYC-03`. |
| _(11)_ | _BR-18.8_ | **Atomic Self-Assignment:**<br>❖ Status update and `shipperId` assignment execute in a single SQL `UPDATE` guarded by optimistic locking.<br>❖ For role `shipper`, `shipperId` is set to `session.user.id`. For role `admin`, `shipperId` is set to the admin's id.<br>❖ After `OrderLifecycleService.pickup(id, session)` succeeds, order assigned to shipper atomically. |
...
| _(14)_ | _BR-18.11_ | **Status Changed Event Publication:**<br>❖ `OrderStatusChangedEvent` is published after commit carrying the transition details and order snapshot.<br>❖ `EventPublisher.publishOrderStatusChangedEvent(orderId, oldStatus, newStatus)` publishes the corresponding domain event. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-19.1_ | **Shipper Role Authorization:**<br>❖ T-10 and T-11 are restricted to roles `shipper` and `admin`; other roles return HTTP 403 referencing `MSG-LCYC-02`.<br>❖ If `OrderLifecycleService.assertShipperRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-03`. |
| _(5)_ | _BR-19.2_ | **Assigned-Shipper Ownership Check:**<br>❖ For role `shipper`, `orders.shipperId` must equal `session.user.id`; otherwise HTTP 403 referencing `MSG-DEL-03`.<br>❖ For role `admin`, the assigned-shipper check is bypassed.<br>❖ If `OrderLifecycleService.assertOrderAssignedToShipper(orderId, shipperId)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-04`. |
...
| _(8)_ | _BR-19.8_ | **T-10 Event Publication:**<br>❖ Successful T-10 publishes `OrderStatusChangedEvent` with `fromStatus = 'picked_up'`, `toStatus = 'delivering'` after commit.<br>❖ No refund event is triggered; T-12 (post-delivery dispute) is admin-only and out of scope.<br>❖ `EventPublisher.publishOrderStatusChangedEvent(orderId, oldStatus, newStatus)` publishes the corresponding domain event. |
...
| _(6)_ | _BR-20.4_ | **Canonical Order Header:**<br>❖ `GET /orders/:id` returns the authoritative order record: `id`, `status`, `paymentMethod`, `totalAmount`, `shipperId`, `version`, and persisted line items.<br>❖ When `OrderHistoryService.getMyOrders(session, filters)` executes, canonical order summary returned. |
| _(7)_ | _BR-20.5_ | **Chronological Timeline:**<br>❖ `GET /orders/:id/timeline` returns the ordered projection of `order_status_logs` rows (`fromStatus`, `toStatus`, `triggeredByRole`, `note`, `createdAt`) for step-by-step progress rendering.<br>❖ When `OrderHistoryService.getTimeline(id, session)` executes, chronological status timeline returned. |
| _(3)_ | _BR-20.6_ | **WebSocket Authentication & Routing:**<br>❖ The Notification WebSocket gateway authenticates via Bearer token and joins the socket to a per-user room based on `session.user.id`.<br>❖ If `NotificationGateway.authenticateGatewayClient(socket)` fails the authorization check, system returns HTTP 403 referencing `MSG-WS-01` (socket authenticated and joined to user room). |
| _(8)_ | _BR-20.7_ | **Real-Time Event Dispatch:**<br>❖ For every `OrderStatusChangedEvent` published after a successful transition (T-01 through T-12), the Notification BC dispatches a `notification_payload` to the customer's room.<br>❖ The `notification_type` is mapped from the `(fromStatus, toStatus)` pair (see UC-26).<br>❖ After `NotificationGateway.broadcastOrderStatusChanged(orderId, userId, status)` executes, real-time event pushed to subscribed clients. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-21.1_ | **Session Role Mapping:**<br>❖ The endpoint is callable by any authenticated session; role-resolution maps to the most-privileged role (admin > restaurant > shipper > customer).<br>❖ If `OrderLifecycleService.assertCustomerRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-05`. |
| _(5)_ | _BR-21.2_ | **Customer Ownership Gate:**<br>❖ For role `customer`, `orders.customerId` MUST equal `session.user.id`; otherwise HTTP 404 referencing `MSG-HIST-01` (no order-existence leak).<br>❖ If `OrderLifecycleService.assertOrderOwnedByCustomer(orderId, customerId)` fails the authorization check, system returns HTTP 403 referencing `MSG-LCYC-AUTH-06`. |
...
| _(7)_ | _BR-21.5_ | **Non-Empty Reason Validation:**<br>❖ Both T-03 and T-05 set `requireNote = true` in the transition map.<br>❖ Requests with missing or whitespace-only `reason` return HTTP 400 referencing `MSG-CANC-01`.<br>❖ If `OrderLifecycleService.validateCancelReason(dto)` returns false, system returns an error response referencing `MSG-LCYC-CAN-01`. |
...
| _(8)_ | _BR-21.7_ | **Cancellable Window (Pending):**<br>❖ A customer may cancel while `order.status = 'pending'` (before payment or confirmation).<br>❖ This maps to T-03: `pending → cancelled`.<br>❖ After `OrderLifecycleService.cancelOrder(id, session, dto)` succeeds, order transitions to `cancelled` when within cancellable window. |
...
| _(11)_ | _BR-21.10_ | **Optimistic Lock Contention:**<br>❖ The status update, version bump and audit-log insert run inside a single DB transaction with `UPDATE orders SET status = 'cancelled', version = version + 1 WHERE id = $1 AND version = $expected`.<br>❖ Lock-loss (concurrent accept by restaurant) returns HTTP 409 referencing `MSG-LCYC-06`; client SHOULD refresh and re-evaluate eligibility.<br>❖ If `LockService.acquireOrderLock(orderId)` returns false, system returns HTTP 409 referencing `MSG-LCYC-03`. |
...
| _(13)_ | _BR-21.12_ | **VNPay Refund Event (T-05):**<br>❖ When source status is `paid` AND `paymentMethod = 'vnpay'`, publish `OrderCancelledAfterPaymentEvent` with: `orderId`, `customerId`, `paymentMethod: 'vnpay'`, `paidAmount`, `cancelledAt`, `cancelledByRole: 'customer'`.<br>❖ `EventPublisher.publishOrderCancelledAfterPaymentEvent(orderId, refundAmount)` publishes the corresponding domain event. |
...
| _(12)_ | _BR-21.14_ | **Notification Fan-out Events:**<br>❖ `OrderStatusChangedEvent` is published triggering Notification BC fan-out: channels `in_app`, `push`, `email` for `order_cancelled` recipient `customer`.<br>❖ `EventPublisher.publishOrderStatusChangedEvent(orderId, oldStatus, newStatus)` publishes the corresponding domain event. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-22.1_ | **Request Validation:**<br>❖ `stars` MUST be an integer in `[1, 5]`; `comment` is optional and bounded to 1 000 UTF-8 characters.<br>❖ The Review BC trims surrounding whitespace from `comment` and rejects empty-after-trim strings as `null`.<br>❖ Validation failures return HTTP 400 referencing `MSG-RATE-01`.<br>❖ If `ReviewService.validateReviewDto(dto)` returns false, system returns an error response referencing `MSG-REVIEW-01`. |
...
| _(7)_ | _BR-22.3_ | **Ownership Verification:**<br>❖ The review handler reads the target order from the Ordering BC's public read API (or a cached `(orderId → customerId, restaurantId, status)` snapshot).<br>❖ The order's `restaurantId` is **copied** onto the `reviews` row at creation time so that later restaurant deletion or reassignment cannot orphan the review.<br>❖ If `OrderLifecycleService.assertOrderOwnedByCustomer(orderId, customerId)` fails the authorization check, system returns HTTP 403 referencing `MSG-REVIEW-02`. |
...
| _(10)_ | _BR-22.8_ | **Uniqueness Constraint:**<br>❖ A UNIQUE constraint on `(orderId, customerId)` enforces a single review row per order at the database layer.<br>❖ The constraint is created at migration time and cannot be circumvented by concurrent inserts.<br>❖ Review editing uses `PATCH /reviews/:id` (subject to BR-22.13), not re-`POST`.<br>❖ If `ReviewRepository.findReviewByOrder(orderId)` returns null or empty, system returns an error response referencing `MSG-REVIEW-04` (review already submitted for this order). |
...
| _(12)_ | _BR-22.11_ | **Transactional Publication:**<br>❖ The review insert and the `RestaurantRatingChangedEvent` publish run in the same DB transaction (outbox or transactional EventBus).<br>❖ If the event publish fails, the entire transaction rolls back and no review row is persisted.<br>❖ Guarantees that no review exists without a corresponding rating-update event in the event log.<br>❖ After `ReviewService.submitOrderReview(orderId, customerId, rating, comment)` completes, review persisted and projection scheduled. |
| _(13)_ | _BR-22.12_ | **Restaurant Rating Projection:**<br>❖ The Restaurant Catalog BC subscribes to `RestaurantRatingChangedEvent` and updates `restaurants.ratingSum`, `restaurants.ratingCount` and the derived `averageRating` projection.<br>❖ The Review BC NEVER writes to the `restaurants` table directly (D-P7 boundary rule).<br>❖ This ensures the restaurant rating is the single source of truth and remains consistent across bounded contexts.<br>❖ After `ReviewService.recomputeRestaurantRating(restaurantId)` succeeds, restaurant aggregate rating recomputed. |
| _(12)_ | _BR-22.13_ | **Moderation-Ready Schema:**<br>❖ `reviews.status ∈ {'published', 'hidden', 'removed'}` — all new reviews enter with `status = 'published'`.<br>❖ Customer endpoints only operate on `published` rows; administrators can transition to `hidden` (visible to author only) or `removed` (soft-delete).<br>❖ A `PATCH /reviews/:id` endpoint allows the original author to edit `stars` / `comment` while `status = 'published'`; non-owners receive `MSG-RATE-06`.<br>❖ After `ReviewService.moderateReview(reviewId, adminId, action)` succeeds, moderation action applied to review record. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(3)_ | _BR-23.1_ | **Role & Restaurant Ownership Gate:**<br>❖ Every mutating route requires role `restaurant` AND `restaurants.ownerUserId = session.user.id` AND `approvalStatus = 'approved'`.<br>❖ The owner can only operate on rows where `promotions.scope = 'restaurant'` AND `promotions.restaurantId` equals an owned restaurant.<br>❖ Cross-restaurant access returns HTTP 403 referencing `MSG-PROMO-02`.<br>❖ If `RestaurantService.assertRestaurantOwnership(restaurantId, session)` fails the authorization check, system returns HTTP 403 referencing `MSG-PROMO-AUTH-01`. |
...
| _(17)_ | _BR-23.13_ | **Response Detail & Message References:**<br>❖ Single promotion responses include all metadata: `type`, `discountValue`, `status`, `trigger`, `activationWindow`, usage metadata.<br>❖ Success responses reference appropriate message codes: `MSG-PROMO-01` (validation error), `MSG-PROMO-05` (state error).<br>❖ List operations return the full promotion objects, not summaries.<br>❖ After `PromotionService.createCoupons(id, session, dto)` completes, generated coupon codes returned to caller. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(3)_ | _BR-24.1_ | **Admin Role Gate:**<br>❖ Every `/promotions/admin/*` endpoint enforces role `admin` via `@Roles(['admin'])` guard.<br>❖ Non-admin sessions receive HTTP 403; the guard fires before payload binding so no resource existence is leaked.<br>❖ Admin identity is logged on every mutation for audit trail.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-PROMO-AUTH-02`. |
...
| _(14)_ | _BR-24.3_ | **Admin Promotion Creation:**<br>❖ Admins can create with `scope = 'platform'` (and `restaurantId = NULL`) or `scope = 'restaurant'` (with a specific `restaurantId`).<br>❖ The same payload validation as UC-23 BR-23.4 applies; failures return `MSG-PROMO-01`.<br>❖ Platform promotions are eligible for all orders regardless of restaurant.<br>❖ After `RestaurantService.create(dto)` completes, platform promotion persisted. |
| _(14)_ | _BR-24.4_ | **Admin Promotion Updates:**<br>❖ Update / lifecycle endpoints honour the same state machine as UC-23 BR-23.7.<br>❖ Admins bypass the `startsAt ≤ now()` activation constraint that UC-23 enforces, allowing past-dated promotions to be activated for corrective workflows.<br>❖ Disallowed transitions still return `MSG-PROMO-05` regardless of admin role.<br>❖ After `RestaurantService.update(id, dto)` completes, platform promotion updated. |
| _(16)_ | _BR-24.5_ | **Status Transition Enforcement:**<br>❖ Admins can also execute `{active, paused} → cancelled` transitions via `DELETE /promotions/admin/:id`.<br>❖ Restaurant owners cannot cancel; only admins have this endpoint.<br>❖ Cancellation is idempotent: cancelling an already-cancelled promotion returns HTTP 204 No Content silently.<br>❖ After `PromotionService.activate(id)` and `PromotionService.pause(id)` succeeds, promotion state transitions per requested action. |
| _(17)_ | _BR-24.6_ | **Atomic Soft-Delete Operation:**<br>❖ `DELETE` transitions the row to `status = 'cancelled'` and returns HTTP 204 No Content — never returns the row body.<br>❖ Historical `promotion_usages` rows continue referencing the cancelled promotion so reporting of past orders remains accurate.<br>❖ Coupon codes attached to a cancelled promotion are NOT auto-revoked.<br>❖ After `PromotionService.cancel(id)` succeeds, promotion soft-deleted atomically. |
...
| _(10)_ | _BR-24.10_ | **Coupon Batch Insertion:**<br>❖ Up to 200 codes may be submitted per request; exceeding this returns HTTP 413 (Payload Too Large).<br>❖ Each inserted row carries `promotionId`, `status = 'active'`, `maxUsesPerCode` (optional), `expiresAt` (optional), and `currentUses = 0`.<br>❖ All inserts run in a single transaction; a single duplicate causes the ENTIRE batch to fail.<br>❖ After `PromotionService.createCoupons(id, dto)` completes, coupon batch inserted in a single transaction. |
...
| _(9)_ | _BR-24.13_ | **Coupon Validation Contract (runtime):**<br>❖ A coupon is valid at checkout only when: `status = 'active'`, `expiresAt > now()`, `currentUses < maxUsesPerCode`, and parent promotion is also active.<br>❖ Any other state returns `MSG-PROMO-06` (invalid / expired / exhausted).<br>❖ Coupon scope is inherited from the parent: `scope = 'restaurant'` codes are rejected when the cart belongs to a different restaurant.<br>❖ If `PromotionService.validateCoupon(code)` returns false, system returns an error response referencing `MSG-PROMO-VAL-03` (coupon code invalid or expired). |
...
| _(10)_ | _BR-25.11_ | **Optimistic Lock Acquisition:**<br>❖ The handler transitions the row to `refund_pending` with `refundInitiatedAt = now()` under optimistic lock.<br>❖ Lock loss (another handler beat this one) causes silent exit; the winner will complete the refund.<br>❖ Version is incremented automatically by the database on every UPDATE.<br>❖ If `LockService.acquirePaymentLock(transactionId)` returns false, system returns HTTP 409 referencing `MSG-PAY-08`. |
| _(14)_ | _BR-25.12_ | **Refund Completion:**<br>❖ A successful VNPay gateway response advances the transaction to `refunded` with `refundedAt = now()`.<br>❖ The UPDATE again uses optimistic locking to detect concurrent completion attempts.<br>❖ Success is logged at INFO level with the refund transaction ID from VNPay for audit trail.<br>❖ After `PaymentService.refund(id, session, dto)` succeeds, refund transaction transitions to `completed`. |
...
| _(14)_ | _BR-25.16_ | **Refund Confirmation Persistence:**<br>❖ The response transaction ID from VNPay is stored in `refundTransactionId` column for later reconciliation with bank statements.<br>❖ The full response timestamp is recorded as `refundedAt`.<br>❖ Database transaction is committed immediately after the gateway call succeeds.<br>❖ After `PaymentService.recordRefundTransaction(orderId, amount, gatewayRef)` completes, refund transaction persisted with gateway reference. |
...
| _(8)_ | _BR-26.10_ | **Quiet Hours Calculation:**<br>❖ Quiet hours are timezone-aware; computed via `QuietHoursService` using the recipient's configured timezone.<br>❖ Quiet hours suppress ONLY the `push` channel; `in_app` is always persisted so the inbox remains complete.<br>❖ Comparison is: `startTime ≤ currentTime (in user's TZ) < endTime`.<br>❖ If `QuietHoursService.isWithinQuietHours(userId, now)` returns false, system returns an error response (delivery suppressed when within quiet-hours window). |
...
| _(18)_ | _BR-26.19_ | **Paginated Inbox REST Contract:**<br>❖ `GET /notifications/my` is paginated (default 20, max 100) with `unreadOnly` and `type` filters; response includes running `unreadCount`.<br>❖ `unreadCount` is cached in Redis (5-minute TTL) and invalidated on every read-state mutation.<br>❖ Non-existent ids in read endpoints return HTTP 404 referencing `MSG-NOTI-02`.<br>❖ When `NotificationService.getInbox(session, query)` executes, paginated inbox returned to caller. |
| _(19)_ | _BR-26.20_ | **Read-State Synchronisation:**<br>❖ `PATCH /notifications/:id/read` (single) and `PATCH /notifications/my/read-all` (bulk) emit WebSocket `notification_read` events for instant convergence across open tabs.<br>❖ Success references `MSG-NOTI-03` (single) or `MSG-NOTI-04` (bulk).<br>❖ All read queries are scoped by `session.user.id` — cross-user access is impossible.<br>❖ After `NotificationService.markRead(session, id)` and `NotificationService.markAllRead(session)` completes, read flag persisted on notification rows. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-27.1_ | **Admin Role Authorization:**<br>❖ Both `PATCH /restaurants/:id/approve` and `PATCH /restaurants/:id/unapprove` require role `admin` via `@Roles(['admin'])`.<br>❖ Non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`; the guard fires before resource lookup so existence is not leaked.<br>❖ Admin identity is logged on the restaurant row for audit trail.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(9)_ | _BR-27.2_ | **Approval Decision Execution:**<br>❖ The decision writes `isApproved` (boolean) atomically to the restaurant row within a single DB transaction.<br>❖ Concurrent approval/unapproval attempts use optimistic locking to prevent race conditions.<br>❖ The `RestaurantUpdatedEvent` is published synchronously after the UPDATE succeeds.<br>❖ After `RestaurantService.approve(id)` and `RestaurantService.unapprove(id)` succeeds, approval state toggled per admin decision. |
| _(1)_ | _BR-27.3_ | **Approval Queue Filtering:**<br>❖ The approval queue is derived from `GET /restaurants` filtered by `isApproved = false` for **Pending** tab, and `isApproved = true` for **Approved** tab.<br>❖ For each row: restaurant name, owner email, submission timestamp, address, phone, and resolved image URLs for logo and cover.<br>❖ Pagination defaults to 20 rows (max 100); the response references `MSG-ADM-01`.<br>❖ When `RestaurantService.listPendingRestaurants(filters)` executes, pending restaurant queue returned. |
...
| _(10)_ | _BR-27.10_ | **Event Publication for State Changes:**<br>❖ Every actual state change publishes `RestaurantUpdatedEvent` consumed by the Ordering ACL projector.<br>❖ The event carries the new `isApproved` value and restaurant ID.<br>❖ Ordering BC caches this change; discovery results reflect it on the next query.<br>❖ `EventPublisher.publishRestaurantUpdatedEvent(restaurantId)` publishes the corresponding domain event. |
| _(11)_ | _BR-27.11_ | **Audit Trail Metadata:**<br>❖ The decision writes `approvalDecidedAt` (timestamp) and `approvalDecidedBy` (admin `userId`) onto the restaurant row.<br>❖ Repeated decisions overwrite the previous decision metadata; full history is reconstructable from the application log (`request_id` correlation).<br>❖ These fields enable compliance audits and traceability of all approval decisions.<br>❖ After `AdminAuditService.appendApprovalAudit(restaurantId, adminId, decision)` completes, approval audit row persisted. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-28.1_ | **Admin Role Enforcement:**<br>❖ The shipper approval endpoints (`PATCH /admin/shipper-applications/:id/approve` and `…/reject`) require role `admin` via `@Roles(['admin'])`.<br>❖ Non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ Admin identity is recorded on the decision for audit trail.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
...
| _(1)_ | _BR-28.3_ | **Approval Queue Composition:**<br>❖ The queue is derived from `GET /admin/shipper-applications` filtered by `status` query parameter (`pending_approval`, `approved`, `rejected`).<br>❖ Each row exposes: applicant email, full name, vehicle type, licence-plate number, submission timestamp, and resolved image URLs for national-ID and driving-licence.<br>❖ Sensitive identifiers (national ID, licence number) are masked except for the last four characters.<br>❖ When `ShipperOnboardingService.listPendingShippers(filters)` executes, pending shipper queue returned. |
...
| _(3)_ | _BR-28.5_ | **Detailed Application Inspection:**<br>❖ When the admin selects an application for detailed review, full applicant info is returned: national ID (masked), driving-licence number (masked), vehicle details.<br>❖ Image references (national ID document, driving-licence document, vehicle photo) are resolved to CDN URLs for browser display.<br>❖ Submission metadata (timestamp, IP address) is included for forensic investigation.<br>❖ When `ShipperOnboardingService.getShipperApplicationDetail(shipperId)` executes, full shipper application detail returned. |
...
| _(9)_ | _BR-28.8_ | **Rejection Reason Validation:**<br>❖ Reject requires a non-empty `reason` (1–500 chars, trimmed). Whitespace-only strings are rejected after trim.<br>❖ Missing or invalid reason returns HTTP 400 referencing `MSG-LCYC-05`.<br>❖ Approve does NOT require a reason; an optional audit note may still be persisted.<br>❖ If `ShipperOnboardingService.rejectShipperRegistration(shipperId, adminId, reason)` returns false, system returns an error response referencing `MSG-SHIP-REG-03` (rejection reason missing). |
...
| _(11)_ | _BR-28.11_ | **Atomic Role Elevation:**<br>❖ Approval updates the application status to `approved` AND elevates the user's role to `shipper` in a single transaction.<br>❖ If role update fails (e.g., user deleted concurrently), the entire decision is rolled back.<br>❖ After commit, `ShipperApprovedEvent` is published asynchronously.<br>❖ After `ShipperOnboardingService.approveShipper(shipperId, adminId, decision)` completes, shipper role and approval status persisted atomically. |
...
| _(18)_ | _BR-28.15_ | **Audit Trail Recording:**<br>❖ Every decision writes `decidedBy` (admin `userId`), `decidedAt` (timestamp) and (for rejection) `rejectionReason` onto the `shipper_applications` row.<br>❖ Application logs preserve a `request_id` correlation for forensic tracing of all decisions.<br>❖ These fields enable compliance audits and historical traceability of shipper onboarding decisions.<br>❖ After `AdminAuditService.appendApprovalAudit(shipperId, adminId, decision)` completes, approval audit row persisted. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-29.1_ | **Authorization Rules:**<br>❖ Both endpoints (`PATCH /admin/users/:userId/suspend` and `PATCH /admin/users/:userId/reactivate`) require role `admin` via `@Roles(['admin'])`.<br>❖ Non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(6)_ | _BR-29.2_ | **Existence Rules:**<br>❖ The target user account is loaded from the repository by `userId`.<br>❖ If the account does not exist, HTTP 404 is returned referencing `MSG-ADM-13`.<br>❖ If `PartnerSuspensionService.findPartnerById(partnerId, partnerType)` returns null or empty, system returns an error response referencing `MSG-PARTNER-01` (partner not found). |
| _(9)_ | _BR-29.3_ | **Validate Rules:**<br>❖ `reason` is required and must be a trimmed, non-empty string of length 1–500.<br>❖ `expiresAt` (optional) — when present — MUST be later than the current timestamp; a past or current timestamp rejects with HTTP 400 referencing `MSG-ADM-12`.<br>❖ If `PartnerSuspensionService.validateSuspensionDto(dto)` returns false, system returns an error response referencing `MSG-PARTNER-02`. |
...
| _(11)_ | _BR-29.5_ | **Persistence Rules:**<br>❖ The transaction updates `users.banned = true`, `users.banReason`, `users.banExpires` and stamps `users.bannedAt` / `users.bannedBy` (admin `userId`).<br>❖ Absence of `expiresAt` indicates an indefinite suspension; the account remains suspended until explicit reactivation.<br>❖ After `PartnerSuspensionService.suspendPartner(partnerId, adminId, reason, duration)` completes, partner marked `suspended` with reason and duration. |
| _(12)_ | _BR-29.6_ | **Session Termination Rules:**<br>❖ All Better Auth sessions for the target user are invalidated immediately.<br>❖ On the next request, the auth layer rejects the session and returns HTTP 401 referencing `MSG-AUTH-05`.<br>❖ After `PartnerSuspensionService.revokePartnerSessions(partnerId)` succeeds, active partner sessions revoked. |
| _(13)_ | _BR-29.7_ | **Notification Rules:**<br>❖ A `system_announcement` notification carrying `reason` and `expiresAt` (when present) is dispatched via the Notification BC (UC-26); this is a **critical type** that bypasses quiet hours and mutes (see BR-26.6).<br>❖ The notification describes the action and (when temporary) the expiry.<br>❖ `EventPublisher.publishPartnerSuspendedEvent(partnerId, partnerType)` publishes the corresponding domain event. |
...
| _(17)_ | _BR-29.9_ | **Reactivation Eligibility:**<br>❖ Reactivation requires the target account's `banned` flag to be `true`.<br>❖ If the account is already in `banned = false`, reactivation returns HTTP 200 with the unchanged row and `MSG-ADM-10` (no-change acknowledgement).<br>❖ After `PartnerSuspensionService.liftSuspension(partnerId, adminId, partnerType)` succeeds, partner suspension lifted. |
...
| _(22)_ | _BR-29.14_ | **Audit Log Rules:**<br>❖ Every decision is recorded with admin `userId`, decision timestamp, action (`suspend` / `reactivate`), `reason` (suspend), `expiresAt` (suspend) on the user row.<br>❖ The history of multiple suspend/reactivate cycles is preserved through the request-correlated application log.<br>❖ After `AdminAuditService.appendSuspensionAudit(partnerId, adminId, action)` completes, suspension audit row persisted. |
| _(18)_ | _BR-29.15_ | **Automatic Expiry:**<br>❖ When `banExpires` is set and `now() ≥ banExpires`, `AccountSuspensionExpiryTask` automatically reactivates the account using the same code path as UC-29 with the system actor.<br>❖ Session validation also treats expired suspensions as inactive so expired suspensions do not block users while expiry reconciliation is running.<br>❖ After `PartnerSuspensionTask.suspensionExpiryTask()` succeeds, expired suspensions automatically lifted by scheduler. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-30.1_ | **Authorization Rules:**<br>❖ All `/admin/orders*` endpoints require role `admin`; the controller additionally re-asserts `hasRole(session.user.role, 'admin')` and throws `ForbiddenException` if the assertion fails.<br>❖ Non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(2)_ | _BR-30.2_ | **Composite Filter Contract:**<br>❖ The query DTO extends the customer history filter DTO with admin-only fields: `restaurantId`, `customerId`, `shipperId`, `paymentMethod`, `sortBy` (`created_at` / `updated_at` / `total_amount`) and `sortOrder` (`asc` / `desc`).<br>❖ All filters compose conjunctively (`AND`); empty filter values are ignored.<br>❖ If `OrderHistoryService.validateOrderFilters(filters)` returns false, system returns an error response referencing `MSG-ORD-MON-01`. |
| _(6)_ | _BR-30.3_ | **Cross-Tenant Read:**<br>❖ Admin reads bypass every ownership filter applied to the same data by UC-10 (customer scope), UC-14 (restaurant scope) and UC-19 (assigned-shipper scope).<br>❖ The admin sees the global cross-tenant view in a single query.<br>❖ When `OrderHistoryService.getAllOrders(filters)` executes, admin sees orders across all tenants. |
...
| _(9)_ | _BR-30.13_ | **Audit Preservation:**<br>❖ All audit logs preserve the read-only request via `request_id` correlation but no `order_status_logs` row is written for read access.<br>❖ Read patterns are logged separately in application telemetry for performance analysis.<br>❖ After `OrderRepository.appendOrderAudit(orderId, actorId, action)` completes, audit row appended for read-side action. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(4)_ | _BR-31.1_ | **Authorization Rules:**<br>❖ All `/admin/users*` endpoints require role `admin`; non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ The admin scope bypasses the per-actor session filters used by Better Auth's self-service endpoints.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
...
| _(2)_ | _BR-31.3_ | **Search Filter Contract:**<br>❖ Filters: `role` (single value), `status` (`active` / `suspended`), `registeredFrom` / `registeredTo` (ISO dates, `from ≤ to`), `q` (free-text on `email` or `name`, 1–100 chars, case-insensitive prefix match).<br>❖ Pagination defaults: `limit = 20`, `max = 100`, `offset ≥ 0`.<br>❖ When `UserAdminService.searchUsers(query, filters)` executes, matching users returned. |
| _(3)_ | _BR-31.4_ | **Filter Validation:**<br>❖ Invalid filter values return HTTP 400 referencing `MSG-ADM-18`.<br>❖ Free-text queries exceeding 100 characters are truncated with a warning in the response.<br>❖ If `UserAdminService.validateUserSearchFilters(filters)` returns false, system returns an error response referencing `MSG-USER-ADMIN-01`. |
...
| _(9)_ | _BR-31.7_ | **Detail Load Precondition:**<br>❖ When drilling into a single user, the endpoint verifies admin role again before loading profile data.<br>❖ The user row is loaded from the database with all profile metadata.<br>❖ If `UserRepository.findUserById(userId)` returns null or empty, system returns an error response referencing `MSG-USER-ADMIN-02` (user not found). |
...
| _(14)_ | _BR-31.11_ | **Action Routing:**<br>❖ Administrator chooses one of: edit profile, suspend/reactivate (UC-29), or change role (UC-35).<br>❖ Suspension, reactivation and role changes are routed to their respective UCs.<br>❖ After `UserAdminService.lockUserAccount(userId, adminId, reason)` succeeds, admin-routed action applied to user account. |
...
| _(16)_ | _BR-31.13_ | **Profile Edit Validation:**<br>❖ Invalid payloads return HTTP 400 referencing `MSG-ADM-18`; conflicts on `contactEmail` return HTTP 409 referencing `MSG-AUTH-02`.<br>❖ Whitespace-only `name` and `notes` values are treated as empty and allowed (stored as empty string).<br>❖ If `UserAdminService.validateUserProfileDto(dto)` returns false, system returns an error response referencing `MSG-USER-ADMIN-03`. |
| _(17)_ | _BR-31.14_ | **Profile Persistence:**<br>❖ Updated fields are saved atomically; partial updates (subset of fields) are permitted.<br>❖ `updatedBy` and `updatedAt` are stamped with the admin actor and decision timestamp.<br>❖ After `UserAdminService.updateUserProfile(userId, patch, adminId)` completes, user profile patch persisted. |
...
| _(20)_ | _BR-31.17_ | **Delegation Boundary:**<br>❖ Suspension and reactivation MUST be performed via the UC-29 endpoints; UC-31 never writes `banned` / `banReason` / `banExpires` directly.<br>❖ Role assignment MUST be performed via the UC-35 endpoint; UC-31 never writes `role` directly.<br>❖ After `UserAdminService.resetUserPassword(userId, adminId)` completes, password reset issued by admin delegate. |
| _(17)_ | _BR-31.18_ | **Audit Trail:**<br>❖ Every profile edit records admin `userId`, decision timestamp and the set of changed fields on the user row's `updatedBy` / `updatedAt` metadata.<br>❖ Application logs preserve the `request_id` correlation.<br>❖ After `AdminAuditService.appendUserAccountAudit(userId, adminId, action)` completes, user-account audit row persisted. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-32.1_ | **Authorization Rules:**<br>❖ Admin cancellation uses the same `PATCH /orders/:id/cancel` endpoint as UC-21; the role `admin` is one of the `allowedRoles` declared on T-03, T-05 and T-07 in the canonical `TRANSITIONS` map.<br>❖ Non-admin sessions receive HTTP 403 referencing `MSG-LCYC-02`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
...
| _(9)_ | _BR-32.3_ | **Reason Requirement:**<br>❖ Both admin actions require a non-empty `reason` (1–500 chars, trimmed); whitespace-only or missing reason returns HTTP 400 referencing `MSG-LCYC-05`.<br>❖ The reason is persisted in the `order_status_logs.notes` column.<br>❖ If `OrderLifecycleService.validateCancelReason(dto)` returns false, system returns an error response referencing `MSG-LCYC-CAN-01`. |
...
| _(11)_ | _BR-32.5_ | **Lifecycle Application:**<br>❖ Admin transitions reuse `TransitionOrderCommand` / `TransitionOrderHandler` and therefore obey the same optimistic-lock contract documented in UC-14 BR-14.x and UC-21 BR-21.x.<br>❖ Every successful transition publishes `OrderStatusChangedEvent` with `actorRole = 'admin'`.<br>❖ After `OrderLifecycleService.cancelOrder(id, session, dto)` succeeds, order transitions to `cancelled` by admin decision. |
| _(12)_ | _BR-32.6_ | **Audit Recording:**<br>❖ An `order_status_logs` row is written with the admin actor (`userId`), decision timestamp, new state, and reason note.<br>❖ The optimistic-lock version is incremented so subsequent operations on the same order detect concurrent modification.<br>❖ After `OrderRepository.appendOrderHistory(orderId, status, actorId)` completes, cancellation audit row persisted. |
| _(13)_ | _BR-32.7_ | **Notification Dispatch:**<br>❖ Downstream Notification BC fan-out (UC-26) honours the same channel mapping (BR-26.4) used by customer- or restaurant-initiated transitions.<br>❖ The customer receives notifications in `in_app`, `email` and (for delivery) `push` channels as configured.<br>❖ `EventPublisher.publishOrderStatusChangedEvent(orderId, oldStatus, newStatus)` publishes the corresponding domain event. |
...
| _(15)_ | _BR-32.9_ | **Payment Refund Initiation:**<br>❖ The Payment BC handler processes the `OrderCancelledAfterPaymentEvent` and initiates the refund via VNPay's reversal API.<br>❖ For COD cancellation, no refund event fires (BR-25.1 condition).<br>❖ If `PaymentService.refund(id, session, dto)` returns success, refund flow initiated for paid order; otherwise the system records the failure and surfaces it to the caller. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-33.1_ | **Authorization Rules:**<br>❖ All `/admin/reports/*` endpoints require role `admin`; non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(6)_ | _BR-33.2_ | **Parameter Validation:**<br>❖ `type ∈ {orders, financial, users, promotions}`; `dateFrom` and `dateTo` are required ISO dates and must satisfy `dateFrom ≤ dateTo`; the span MUST NOT exceed 366 days.<br>❖ Optional filters: `restaurantId` (UUID), `paymentMethod ∈ {cod, vnpay}`, `promotionScope ∈ {platform, restaurant}`.<br>❖ If `ReportService.validateReportParameters(reportType, dateRange)` returns false, system returns an error response referencing `MSG-REPORT-01`. |
...
| _(8)_ | _BR-33.5_ | **Report Computation:**<br>❖ **Orders Report** — series: daily counts by status; KPIs: total orders, completion rate (`delivered / created`), cancellation rate, average preparation time (`confirmed → ready_for_pickup`) and delivery time (`picked_up → delivered`).<br>❖ All aggregations filter out cancelled and refunded orders unless explicitly requested.<br>❖ When `ReportService.getOrderStats(startDate, endDate, filters)` executes, aggregated order statistics returned. |
| _(8)_ | _BR-33.6_ | **Financial Report Definition:**<br>❖ **Financial Report** — series: daily GMV (sum of `totalAmount` for `delivered` orders), daily commission (GMV × commission rate, per BR-5), payment-method split, daily refund total (sum of refunded `payment_transactions.amount`); KPIs: period GMV, period commission, refund ratio.<br>❖ Commission is computed as `GMV × commissionRate` where `commissionRate` is the platform-wide fixed percentage configured in `app_settings`.<br>❖ When `ReportService.getRevenueReport(startDate, endDate, restaurantId)` executes, financial report rows returned. |
| _(8)_ | _BR-33.7_ | **Onboarding and Operations Reports:**<br>❖ **Users Report** — series: new registrations per role per day; KPIs: total active, total suspended, pending shipper applications, pending restaurant approvals.<br>❖ **Promotions Report** — series: daily redemption count; KPIs: active promotions, total discount distributed, top-N coupons by redemption.<br>❖ When `ReportService.getShipperPerformanceReport(shipperId, startDate, endDate)` and `ReportService.getRestaurantPerformanceReport(restaurantId, startDate, endDate)` executes, partner performance report returned. |
| _(10)_ | _BR-33.8_ | **Export Contract:**<br>❖ Supported formats: `csv` (UTF-8 with BOM, RFC 4180 quoting) and `xlsx` (Office Open XML, one sheet per series).<br>❖ The exported file contains the exact same rows that the on-screen view rendered for the same parameters — no extra rows or smoothing.<br>❖ After `ReportService.exportReport(reportType, dateRange, format)` completes, export artifact produced in requested format. |
...
| Activity | BR Code | Description |
|---|---|---|
| _(2)_ | _BR-34.1_ | **Authorization Rules:**<br>❖ `GET /admin/dashboard` requires role `admin`; non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(3)_ | _BR-34.2_ | **KPI Catalogue:**<br>❖ **Orders Today** — count grouped by status (`pending`, `paid`, `confirmed`, `preparing`, `ready_for_pickup`, `picked_up`, `delivering`, `delivered`, `cancelled`, `refunded`).<br>❖ **Financial Today** — GMV (sum of `totalAmount` for `delivered` orders today), commission (GMV × `commissionRate`, per BR-5).<br>❖ When `DashboardService.getAdminDashboard()` executes, admin KPI snapshot returned. |
| _(3)_ | _BR-34.3_ | **Onboarding KPIs:**<br>❖ **Onboarding Today** — new registrations by role, pending restaurant approvals (`isApproved = false`), pending shipper applications (`status = 'pending_approval'`).<br>❖ All counts are scoped to the current UTC day (00:00:00–23:59:59 UTC).<br>❖ When `DashboardService.getRestaurantDashboard(restaurantId)` executes, restaurant KPI snapshot returned. |
| _(4)_ | _BR-34.4_ | **Recent Activity Stream:**<br>❖ Last 10 orders by `createdAt`; last 5 restaurant registrations; last 5 shipper applications; last 5 platform-scope promotions.<br>❖ Each entry exposes the minimum fields needed for inline preview plus a deep link to the owning UC's detail view.<br>❖ When `DashboardService.getShipperDashboard(shipperId)` executes, shipper KPI snapshot returned. |
| _(5)_ | _BR-34.5_ | **Snapshot Response:**<br>❖ The dashboard endpoint SHOULD return within 1 second under nominal load; all underlying queries leverage the existing indexes documented in BR-30.9.<br>❖ A successful retrieval references `MSG-ADM-25`.<br>❖ When `DashboardService.getCustomerDashboard(customerId)` executes, customer KPI snapshot returned. |
...
| _(5)_ | _BR-34.7_ | **Caching Strategy:**<br>❖ Identical requests within a 30-second window may be served from an in-memory cache scoped to the admin `userId`; cache is invalidated by any UC-27, UC-28, UC-29, UC-32 write.<br>❖ Cache keys include the admin ID to ensure per-actor isolation.<br>❖ If `DashboardService.getCachedDashboard(scope, key)` returns null or empty, system returns an error response (cached dashboard returned when fresh; otherwise recomputed). |
...
| Activity | BR Code | Description |
|---|---|---|
| _(5)_ | _BR-35.1_ | **Authorization Rules:**<br>❖ `PATCH /admin/users/:userId/role` requires role `admin`; non-admin sessions receive HTTP 403 referencing `MSG-AUTH-05`.<br>❖ If `AdminGuard.assertAdminRole(session)` fails the authorization check, system returns HTTP 403 referencing `MSG-ADMIN-AUTH-01`. |
| _(8)_ | _BR-35.2_ | **Payload Validation:**<br>❖ `role` (primary) MUST be one of `{admin, restaurant, user}`. Direct assignment of `shipper` is rejected with HTTP 400 referencing `MSG-ADM-28`; the `shipper` role is acquired exclusively through UC-28 approval to preserve the application-and-documents audit trail (BR-1).<br>❖ `soliRoles` (optional) MUST be a subset of `{admin, restaurant, shipper, user}` AND MUST contain `role`. When omitted, `soliRoles` is reset to `[role]`.<br>❖ If `RolePermissionService.validateRoleAssignmentDto(dto)` returns false, system returns an error response referencing `MSG-ROLE-01`. |
...
| _(10)_ | _BR-35.4_ | **Last-Administrator Safeguard:**<br>❖ A decision that demotes the only non-suspended `admin` account is rejected with HTTP 409 referencing `MSG-ADM-27`.<br>❖ The safeguard runs inside the same transaction as the role update to defeat any TOCTOU race during simultaneous admin demotions.<br>❖ If `RolePermissionService.assertNotLastAdmin(userId, role)` returns false, system returns an error response referencing `MSG-ROLE-02` (last-administrator safeguard prevents demotion). |
...
| _(6)_ | _BR-35.6_ | **User Existence Check:**<br>❖ The target user is loaded from the repository by `userId`.<br>❖ If the user does not exist, HTTP 404 is returned referencing `MSG-ADM-13`.<br>❖ If `UserRepository.findUserById(userId)` returns null or empty, system returns an error response referencing `MSG-USER-ADMIN-02` (user not found). |
| _(11)_ | _BR-35.7_ | **Atomic Role Persistence:**<br>❖ The transaction commits `role`, `soliRoles`, `roleUpdatedAt`, `roleUpdatedBy` and `roleUpdateReason` atomically.<br>❖ All Better Auth sessions for the target user are refreshed (claims re-issued) so the next request sees the new role set without requiring sign-out.<br>❖ After `RolePermissionService.assignRoleToUser(userId, role, adminId)` and `RolePermissionService.revokeRole(userId, role, adminId)` completes, role assignment persisted atomically. |
| _(12)_ | _BR-35.8_ | **Role Change Event:**<br>❖ A `RoleAssignmentChangedEvent` carrying `userId`, `previousRole`, `previousSoliRoles`, `newRole`, `newSoliRoles`, `actorAdminId` is published.<br>❖ The Notification BC dispatches a `system_announcement` to the target user (critical type, bypasses quiet hours per BR-26.6).<br>❖ `EventPublisher.publishRoleChangedEvent(userId, role, action)` publishes the corresponding domain event. |
| _(13)_ | _BR-35.9_ | **Session Refresh:**<br>❖ All Better Auth sessions for the target user have their JWT claims refreshed in-place where possible; if not possible, existing sessions transition to a `refresh_required` state on the next request.<br>❖ Users see their new role set within one request cycle.<br>❖ After `RolePermissionService.revokeUserSessions(userId)` succeeds, user sessions revoked to force re-authorisation. |
| _(14)_ | _BR-35.10_ | **Audit Trail:**<br>❖ The full before-and-after role set is preserved on the user row for the latest change; historical role transitions are reconstructable from the request-correlated application log.<br>❖ Successful changes reference `MSG-ADM-26`.<br>❖ After `AdminAuditService.auditRoleChange(userId, role, action, adminId)` completes, role-change audit row persisted. |
