    ...
    <View className="flex-1 bg-background" style={{ paddingTop: insets.top }}>
      ...
      >
        ...
        <View className="px-4 mb-4 flex-row justify-between items-center">
          <Text className="text-on-surface-variant text-sm" style={{ fontFamily: 'Inter_600SemiBold' }}>
            Recent Orders ({data?.total || 0})
          </Text>
          <TouchableOpacity onPress={() => refetch()} disabled={isRefetching}>
            <RefreshCcw size={16} color="#0d631b" className={isRefetching ? 'animate-spin' : ''} />
          </TouchableOpacity>
        </View>
        ...
        {isLoading ? (
          <View className="py-20 items-center justify-center">
            ...
            <Text className="mt-4 text-on-surface-variant" style={{ fontFamily: 'Inter_400Regular' }}>
              Loading your orders...
            </Text>
          </View>
        ) : isError ? (
          <View className="py-20 items-center justify-center px-10">
            <Text className="text-error text-center mb-4" style={{ fontFamily: 'Inter_600SemiBold' }}>
            ...
            >
              <Text className="text-on-primary" style={{ fontFamily: 'Inter_700Bold' }}>Retry</Text>
            </TouchableOpacity>
          </View>
        ) : orders.length === 0 ? (
          <View className="py-20 items-center justify-center px-10">
            <Text className="text-on-surface-variant text-center" style={{ fontFamily: 'Inter_400Regular' }}>
          ...
          <View className="flex-col">
            {orders.map((order) => (
              <OrderCard 
                ...
                order={order}
                onActionPress={() => {
                  if (['pending', 'paid', 'confirmed', 'preparing', 'ready_for_pickup', 'picked_up', 'delivering'].includes(order.status)) {
